...
static volatile uint16  time_key_sound;      //按键音定时器
static           uint8  key_sound_type   = SOUND_TYPE_NONE;    //按键音的类型

static volatile uint16  time_alarm_sound;    //报警音计时器
...
void  Set_Default_Bri(void)   //占空比大于50%亮度调整已经无意义
 	...
 	PWM0MR2 = PWM0MR0*10/10;   // 设置无线驱动信号1占空比
 	PWM0MR3 = PWM0MR0*10/10;   // 设置无线驱动信号2占空比
...
{    //PWM1[1]
    ...
    PWM1MR1 = PWM1MR0 / 2;    // 设置PWM占空比50%
    PWM1LER = 0x03;            // PWMMR0, PWMMR2锁存   
}
/****************************************************************************
* 名称：Stop_Sound_PWM()
* 功能：停止声音PWM
...
void Stop_Sound_PWM(void)
{
    ...
    PWM1MR0 = Fpclk / 65536;
    PWM1MR1 = 0;        // 使PWM1一直输出低电平
    PWM1LER = 0x03;    // PWMMR0, PWMMR1锁存  
}
/****************************************************************************
* 名称：Init_Sound_PWM()
* 功能：初始化声音PWM
...
{
    if (OFF == sound_switch)    return;      //声音总开关
...
* 出口参数：无
****************************************************************************/
void Key_Sound_Pro_old(void)
{
       switch(key_sound_type)
        {
         case SOUND_TYPE_KEY_VALID:    //按键有效时发出声音
            if(time_key_sound > 10) Start_Sound_PWM(400);    //发420Hz声音"3"
            else Start_Sound_PWM(300);                        //发333Hz声音"1"
            break;
    ...
    if(SOUND_TYPE_NONE != key_sound_type) return;    //有按键声则退出不处理报警声
     ...
     switch(alarm_sound_type)
     {
        case SOUND_TYPE_WARNING://发报警类声音(10声,周期10秒,3长2短,频率666Hz)
            if(time_alarm_sound < 75) 
               Start_Sound_PWM(666);            //发160ms
            else 
               Stop_Sound_PWM();                        
            /*if(time_alarm_sound < 24) Stop_Sound_PWM();        //停80ms
            ...
            if(++time_alarm_sound >= 150) 
              {
               time_alarm_sound = 0;      //3秒退出
              } 
            break;
        case SOUND_TYPE_TIPS://发提示类报警声(10声,周期10秒,3长2短,频率333Hz)
            if(time_alarm_sound < 100) 
               Start_Sound_PWM(333);            //发160ms
            else 
               Stop_Sound_PWM();
            /*
            if(time_alarm_sound < 24) Stop_Sound_PWM();        //停80ms
            ...
            if(++time_alarm_sound >= 250)
               time_alarm_sound = 0;    //3秒循环
            break;
