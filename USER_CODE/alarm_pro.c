...
//警示类报警 条件消失后删除
Alm_Struct warn_weigh_out    = {ALARM_WEIGH_OUT_WARN,       "重量超载",                             "LOAD OVER",FALSE,NULL};

Alm_Struct warn_wps_out      = {ALARM_WPS_OUT_WARN,         "单件重量超出范围",              "WPS DATA WARNING",FALSE,NULL};
Alm_Struct warn_pcs_out      = {ALARM_PCS_OUT_WARN,         "件数超出范围",                   "PCS OUT WARNING",FALSE,NULL};
Alm_Struct warn_check_out    = {ALARM_CHECK_OUT_WARN,       "检重超出范围",                  "PCS DATA WARNING",FALSE,NULL};
Alm_Struct warn_100_out      = {ALARM_100_OUT_WARN,         "百分比基准点超出范围",    "100 BENCH_DATA WARNING",FALSE,NULL};
Alm_Struct warn_100_data_out = {ALARM_100_TARGET_WARN,      "百分比数据超出范围",            "100 DATA WARNING",FALSE,NULL};

Alm_Struct warn_DS18B20_err  = {ALARM_DS18B20_WARN,         "温度传感器故障",             "TEMP SENSOR INVALID",FALSE,NULL};
Alm_Struct warn_temp_out     = {ALARM_TEMP_OUT_WARN,        "环境温度超出范围",                         "TEMP OVER",FALSE,NULL};
Alm_Struct warn_temp_c_out   = {ALARM_TEMP_CHANGE_OUT_WARN, "温度变化太大 请重新校准",          "PLEASE RE_CAL",FALSE,NULL};
Alm_Struct warn_loadtime_out = {ALARM_LOAD_TIME_OUT_WARN,   "加载超时,请重新启动",    "TIME OUT,PLEASE RESTART",FALSE,NULL};
...
Alm_Struct tips_sample_wps   = {TIPS_SAMPLE_WPS_WARN,       "请采集单件重",                 "PLEASE SAMPLE WPS",FALSE,NULL};
Alm_Struct tips_sample_bench = {TIPS_SAMPLE_BENCH_WARN,     "请采集基准点",               "PLEASE SAMPLE BENCH",FALSE,NULL};
Alm_Struct tips_input_limit  = {TIPS_INPUT_LIMIT_WARN,      "请输入上下限值",              "PLEASE INPUT LIMIT",FALSE,NULL};

Alm_Struct tips_cal_stop     = {TIPS_CAL_STOP_WARN,         "校准中或者马达错误",         "CALING OR MOTOR ERR",FALSE,NULL};
...
void Put_Alm_Mess(void)
{
    ...
    if((warning_alm_timer<4)&&(warning_alm->alm_id != NONE_ALM))//有报警信息
    {
         ...
         Rectangle(170,1,455,30);
       
         Set_Chr_Color(COL_WHITE,COL_THINRED);  //红底白字
         Set_ASC_Lib(Arial16B_Font);
         Put_Line_Str(180, 5,Language_Str((uint8*)warning_alm->ch_mess,(uint8*)warning_alm->en_mess));
    }
    else if(warning_alm->alm_id == NONE_ALM)
     {
      Set_Graphic_Color(COL_THINBLUE,COL_THINBLUE);
      Bar(170,1,455,30);  //擦除报警信息
     }
    ...
    if((tips_alm_timer<4)&&(tips_alm->alm_id != NONE_ALM))//有报警信息
    {
        ...
        Rectangle(170,35,455,65);
     
        Set_Chr_Color(COL_BLACK,COL_THINYELLOW); //黄底黑字
        Set_ASC_Lib(Arial16B_Font);
        Put_Line_Str(180,40,Language_Str((uint8*)tips_alm->ch_mess,(uint8*)tips_alm->en_mess));
    }
    else if(tips_alm->alm_id == NONE_ALM)
      {
       Set_Graphic_Color(COL_THINBLUE,COL_THINBLUE);
       Bar(170,35,455,67);  //擦除报警信息
      }
