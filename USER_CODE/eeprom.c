...
uint8 EEPROM_RW_Test(void)
{
    uint8  test_tx_buf[8];
    uint32 i,j;
    test_tx_buf[0] = 0x12; test_tx_buf[1] = 0x34;
    test_tx_buf[2] = 0x56; test_tx_buf[3] = 0x78;
    test_tx_buf[4] = 0x87; test_tx_buf[5] = 0x65;
    test_tx_buf[6] = 0x43; test_tx_buf[7] = 0x21;
    
    I2C_Send_Str(SYS24C08, EEP_TEST_ADDR, test_tx_buf, 8);
    for(i=0;i<800000;i++)	{;}	             //delay
	for(i=0;i<8;i++) test_tx_buf[i] = 0;	 //clear buf
	
	I2C_Rcv_Str(SYS24C08,  EEP_TEST_ADDR, test_tx_buf, 8);	
	for(i=0;i<80000;i++)	{;}					 //delay
	
	
    
    i = (test_tx_buf[0]<<24)+(test_tx_buf[1]<<16)+(test_tx_buf[2]<<8)+test_tx_buf[3];
    j = (test_tx_buf[4]<<24)+(test_tx_buf[5]<<16)+(test_tx_buf[6]<<8)+test_tx_buf[7];
    
    if((0x12345678==i) && (0x87654321==j))
    {
      /*   配置UART 功能 移到了单独的菜单中处理
      I2C_Rcv_Str(SYS24C08,  EEP_UART_ADDR, test_tx_buf, 1);
      for(i=0;i<80000;i++)	{;}	             //delay
      test_tx_buf[0]++;
	  I2C_Send_Str(SYS24C08, EEP_UART_ADDR, test_tx_buf, 1);
	  for(i=0;i<80000;i++)	{;}	             //delay  */
      return(OK);
    } 
    else
